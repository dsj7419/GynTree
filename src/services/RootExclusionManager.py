...
class RootExclusionManager:
    ...
    def get_root_exclusions(
        self, project_info: Dict[str, bool], start_directory: str
    ) -> Set[str]:
        """
        ...
        for project_type, is_detected in project_info.items():
            if is_detected:
                exclusions.update(
                    self._get_project_type_exclusions(project_type, start_directory)
                )
    ...
    def _get_project_type_exclusions(
        self, project_type: str, start_directory: str
    ) -> Set[str]:
        """
    ...
    def merge_with_existing_exclusions(
        self, existing_exclusions: Set[str], new_exclusions: Set[str]
    ) -> Set[str]:
        """
    ...
    def remove_project_type_exclusion(self, project_type: str, exclusions: Set[str]):
        ...
        if project_type in self.project_type_exclusions:
            self.project_type_exclusions[project_type] -= exclusions
            logger.info(
                f"Removed exclusions for project type {project_type}: {exclusions}"
            )
