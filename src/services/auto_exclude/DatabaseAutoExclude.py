        ...
        for root, dirs, files in self.walk_directory():
            if "prisma" in dirs:
                ...
                if os.path.exists(schema_path):
                    recommendations["excluded_files"].add(
                        os.path.relpath(schema_path, self.start_directory)
                    )
                    logger.debug(
                        f"DatabaseAutoExclude: Recommending exclusion of schema file {schema_path}"
                    )
            ...
            for file in files:
                if file.endswith((".sqlite", ".db", ".sqlite3", ".db3", ".sql")):
                    ...
                    logger.debug(
                        f"DatabaseAutoExclude: Recommending exclusion of database file {full_path}"
                    )
