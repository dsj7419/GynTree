...
class IDEandGitAutoExclude(ExclusionService):
    ...
    def get_exclusions(self) -> Dict[str, Set[str]]:
        ...
        logger.debug(
            f"IDEandGitAutoExclude: Adding common root exclusions: {common_root_exclusions}"
        )
        ...
        logger.debug(
            f"IDEandGitAutoExclude: Recommending common file exclusions: {common_file_exclusions}"
        )
        ...
        for root, dirs, files in self.walk_directory():
            for file in files:
                if file.endswith((".log", ".tmp", ".bak", ".orig", ".user")):
                    ...
                    logger.debug(
                        f"IDEandGitAutoExclude: Recommending exclusion of file {relative_path}"
                    )
