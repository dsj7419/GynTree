...
class AutoExcludeManager:
    def __init__(self, start_directory: str, settings_manager: SettingsManager, 
                 project_types: Set[str], project_type_detector: ProjectTypeDetector):
    ...
    def get_recommendations(self) -> Dict[str, Set[str]]:
        ...
        for service in self.exclusion_services:
            try:
                ...
                logger.error(f"Error getting exclusions from service {type(service).__name__}: {str(e)}")

        # Filter out already excluded items
        new_recommendations = {'root_exclusions': set(), 'excluded_dirs': set(), 'excluded_files': set()}
        
        for category in ['root_exclusions', 'excluded_dirs', 'excluded_files']:
            new_items = self.raw_recommendations[category] - current_exclusions[category]
