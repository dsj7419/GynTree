...
class SettingsManager:
    ...
    def __init__(self, project: Project):
        ...
        self.config_path = os.path.join(self.config_dir, 'projects', f"{self.project.name}.json")
    ...
    def load_settings(self) -> Dict[str, Any]:
        ...
        default_settings = {
            'root_exclusions': list(self.project.root_exclusions) if self.project.root_exclusions else [],
            'excluded_dirs': list(self.project.excluded_dirs) if self.project.excluded_dirs else [],
            'excluded_files': list(self.project.excluded_files) if self.project.excluded_files else [],
    ...
    def is_excluded_dir(self, path: str) -> bool:
        ...
        for excluded_dir in self.get_excluded_dirs():
            ...
            try:
                relative_to_excluded = os.path.relpath(normalized_path, 
                                                    os.path.join(self.project.start_directory, excluded_dir))
                if not relative_to_excluded.startswith('..'):
                    logger.debug(f"Path is inside excluded directory: {path} (inside {excluded_dir})")
                    return True
            except ValueError:
                continue
        return False
    ...
    def is_excluded_file(self, path: str) -> bool:
        ...
        for excluded_file in self.get_excluded_files():
            ...
            if (fnmatch.fnmatch(relative_path, excluded_file) or 
                fnmatch.fnmatch(filename, excluded_file)):
            ...
            if os.sep in excluded_file:
                if fnmatch.fnmatch(relative_path, excluded_file):
                    logger.debug(f"Excluded file (with path): {path} (matched {excluded_file})")
                    return True
                    
            # Handle simple patterns (e.g. *.log)
            elif '*' in excluded_file or '?' in excluded_file:
                if fnmatch.fnmatch(filename, excluded_file):
                    logger.debug(f"Excluded file (pattern): {path} (matched {excluded_file})")
                    return True
