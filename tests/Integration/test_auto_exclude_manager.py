...
def auto_exclude_manager(mock_project, settings_manager, project_type_detector):
    ...
    manager = AutoExcludeManager(
        mock_project.start_directory, settings_manager, set(), project_type_detector
    )
    yield manager
    gc.collect()
...
def test_get_formatted_recommendations(helper, settings_manager, project_type_detector):
    ...
    settings_manager.update_settings(
        {"root_exclusions": [], "excluded_dirs": [], "excluded_files": []}
    )
    ...
    auto_exclude_manager = AutoExcludeManager(
        str(helper.tmpdir), settings_manager, project_types, project_type_detector
    )
...
def test_apply_recommendations(auto_exclude_manager, settings_manager, helper):
    ...
    assert len(updated_settings["root_exclusions"]) >= len(
        initial_settings["root_exclusions"]
    )
...
def test_exclusion_services_creation(helper, settings_manager, project_type_detector):
    ...
    service_names = [
        service.__class__.__name__ for service in manager.exclusion_services
    ]
...
@pytest.mark.timeout(30)
def test_new_exclusions_after_settings_update(
    auto_exclude_manager, settings_manager, helper
):
