...
class ProjectContextTestHelper:
    ...
    def setup_project_files(self, project_type: str) -> None:
        if project_type == "python":
            ...
            (self.tmpdir / "index.html").write_text(
            ...
            (self.tmpdir / "index.js").write_text(
...
def test_initialization(
    project_context: ProjectContext, helper: ProjectContextTestHelper
) -> None:
    helper.track_memory()
...
@pytest.mark.timeout(30)
def test_initialize_root_exclusions(
    project_context: ProjectContext, helper: ProjectContextTestHelper
) -> None:
    helper.track_memory()
...
def test_get_directory_tree(
    project_context: ProjectContext, helper: ProjectContextTestHelper, tmpdir: Path
) -> None:
    helper.track_memory()
    ...
    nested_dir = (
        test_dir / "nested_dir"
    )  # Changed from test_dir to avoid auto-exclusion
...
@pytest.mark.timeout(30)
def test_project_context_with_existing_settings(
    mock_project: Project, helper: ProjectContextTestHelper, tmpdir: Path
) -> None:
    helper.track_memory()
...
@pytest.mark.timeout(30)
def test_project_type_detection_multiple(
    project_context: ProjectContext, helper: ProjectContextTestHelper
) -> None:
    helper.track_memory()
...
@pytest.mark.timeout(30)
def test_auto_exclude_manager_initialization(
    project_context: ProjectContext, helper: ProjectContextTestHelper
) -> None:
    helper.track_memory()
...
@pytest.mark.timeout(30)
def test_directory_analyzer_reinitialize(
    project_context: ProjectContext, helper: ProjectContextTestHelper
) -> None:
    helper.track_memory()
