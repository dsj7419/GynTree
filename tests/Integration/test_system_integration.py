...
class TestSystemIntegration:
    ...
    @pytest.mark.cleanup
    def test_end_to_end_project_workflow(self, qapp, mock_project, qtbot, setup_test_dir, monkeypatch):
    ...
    @pytest.mark.cleanup
    def test_thread_controller_integration(self, qapp, mock_project, qtbot, setup_test_dir, monkeypatch):
        monkeypatch.setattr(QMessageBox, 'warning', mock_msg_box)
        monkeypatch.setattr(QMessageBox, 'critical', mock_msg_box)
    ...
    @pytest.mark.cleanup
    def test_resource_cleanup_integration(self, qapp, mock_project, qtbot, setup_test_dir, monkeypatch):
        monkeypatch.setattr(QMessageBox, 'warning', mock_msg_box)
        monkeypatch.setattr(QMessageBox, 'critical', mock_msg_box)
    ...
    @pytest.mark.cleanup
    def test_project_type_detection_integration(self, qapp, mock_project, setup_test_dir, monkeypatch):
        monkeypatch.setattr(QMessageBox, 'warning', mock_msg_box)
        monkeypatch.setattr(QMessageBox, 'critical', mock_msg_box)
    ...
    @pytest.mark.cleanup
    def test_error_handling_integration(self, qapp, mock_project, setup_test_dir, monkeypatch):
        monkeypatch.setattr(QMessageBox, 'warning', mock_msg_box)
        monkeypatch.setattr(QMessageBox, 'critical', mock_msg_box)
    ...
    @pytest.mark.cleanup
    def test_auto_exclude_integration(self, qapp, mock_project, qtbot, setup_test_dir, monkeypatch):
        monkeypatch.setattr(QMessageBox, 'warning', mock_msg_box)
        monkeypatch.setattr(QMessageBox, 'critical', mock_msg_box)
        ...
        with logger_context() as test_logger:
            ...
            def check_auto_exclude():
                return (controller.project_context and 
                       controller.project_context.auto_exclude_manager and 
                       controller.project_context.auto_exclude_manager.get_recommendations())
            
            assert wait_for_condition(qtbot, check_auto_exclude)
            
            recommendations = controller.project_context.auto_exclude_manager.get_recommendations()
    ...
    @pytest.mark.cleanup   
    def test_settings_integration(self, qapp, mock_project, qtbot, setup_test_dir, monkeypatch):
        monkeypatch.setattr(QMessageBox, 'warning', mock_msg_box)
        monkeypatch.setattr(QMessageBox, 'critical', mock_msg_box)
        ...
        def project_ready():
            return (controller.project_context is not None and 
                    controller.project_context.is_initialized and
                    controller.main_ui.isVisible())
        ...
        qtbot.mouseClick(
            controller.main_ui.theme_toggle, 
            Qt.LeftButton,
            pos=toggle_center
        )
        ...
        print(f"Toggle state after click: {controller.main_ui.theme_toggle.isChecked()}")
        ...
        assert final_toggle_state == (final_theme == 'dark'), "Toggle state doesn't match theme"