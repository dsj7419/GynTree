...
def test_directory_analyzer_memory_usage(
    create_large_directory_structure, settings_manager
):
...
@pytest.mark.timeout(60)
def test_directory_analyzer_performance(
    create_large_directory_structure, settings_manager
):
    ...
    assert execution_time < 15, f"Analysis took {execution_time:.2f} seconds"
    assert len(result["children"]) > 0


@pytest.mark.timeout(120)
def test_directory_analyzer_scalability(
    create_large_directory_structure, settings_manager
):
...
@pytest.mark.timeout(60)
def test_directory_analyzer_with_exclusions(
    create_large_directory_structure, settings_manager
):
...
@pytest.mark.timeout(120)
def test_directory_analyzer_memory_leak(
    create_large_directory_structure, settings_manager
):
