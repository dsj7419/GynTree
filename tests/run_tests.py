...
class TestResult:
    def __init__(self, success: bool = False, message: str = "", duration: float = 0.0,
                 coverage: Optional[float] = None, failed_tests: List[str] = None):
...
class TestRunner:
    ...
    def run_tests(self, options: Dict, selected_tests: Optional[List[str]] = None) -> TestResult:
    ...
    while True:
        ...
        try:
            ...
            if 1 <= choice <= len(options):
                return choice
            print_colored("Invalid choice. Please enter a number within the range.", Fore.RED)
        except ValueError:
...
def main(debug_mode: bool = False, ci_mode: bool = False):
    ...
    while True:
        ...
        if choice == 7:  # Run Single Test
            ...
            test_files = sorted([
                test.resolve() for test in Path('tests').rglob('test_*.py')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--ci", 
        action="store_true",
        help="Run in CI mode with last saved configuration"
    )
    parser.add_argument(
        "--debug", 
        action="store_true",
