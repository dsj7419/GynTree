...
from PyQt5.QtWidgets import (
    QWidget, QLabel, QTreeWidget, QPushButton, 
    QTreeWidgetItem, QHeaderView, QMessageBox, QPushButton, QApplication
)
...
def clean_temp_files():
    ...
    try:
        for filename in os.listdir(original_tempdir):
            if filename.startswith('tmp') and (filename.endswith('.png') or filename.endswith('.txt')):
...
def test_export_functions(directory_tree_ui, qtbot, mocker, tmp_path):
    ...
    mock_info = mocker.patch('PyQt5.QtWidgets.QMessageBox.information', return_value=QMessageBox.Ok)
    mock_error = mocker.patch('PyQt5.QtWidgets.QMessageBox.critical', return_value=QMessageBox.Ok)
    ...
    # Test PNG export
    export_png_btn = next(btn for btn in directory_tree_ui.findChildren(QPushButton) 
                         if btn.text() == 'Export PNG')
    QTest.mouseClick(export_png_btn, Qt.LeftButton)
    qtbot.wait(1000)
    
    # Test ASCII export
    export_ascii_btn = next(btn for btn in directory_tree_ui.findChildren(QPushButton)
    ...
    assert any([mock_info.called, mock_error.called, mock_warning.called]), \
...
def test_export_empty_tree(directory_tree_ui, qtbot, mocker, tmp_path):
    ...
    mock_warning = mocker.patch('PyQt5.QtWidgets.QMessageBox.warning', side_effect=lambda *args: QMessageBox.Ok)
    
    # Use a real temporary file path for export
    png_path = tmp_path / "empty_export.png"
    mock_file_dialog = mocker.patch('PyQt5.QtWidgets.QFileDialog.getSaveFileName', return_value=(str(png_path), 'PNG Files (*.png)'))
...
def test_expand_collapse_functionality(directory_tree_ui, qtbot):
    ...
    # Find buttons
    collapse_btn = next(btn for btn in directory_tree_ui.findChildren(QPushButton)
                       if btn.text() == 'Collapse All')
    expand_btn = next(btn for btn in directory_tree_ui.findChildren(QPushButton)
        ...
        return {
            ...
            'children': [
            ...
            ] + [
                {
                    'name': f'subdir_{i}',
                    'type': 'directory',
                    'children': [] if depth == 1 else create_large_structure(depth - 1, files_per_dir)['children']
                } for i in range(3)
            ]
        }
