...
class ResultUITestHelper:
    ...
    def __init__(self):
        ...
        self.test_data = [
            {"path": "/test/file1.py", "description": "Test file 1 description"},
            {"path": "/test/file2.py", "description": "Test file 2 description"},
        ]
...
def test_copy_to_clipboard(result_ui, qtbot, mocker, helper):
    ...
    copy_btn = next(
        btn
        for btn in result_ui.findChildren(QPushButton)
...
def test_large_dataset(result_ui, qtbot, mock_directory_analyzer, helper):
    ...
    large_data = [
        {"path": f"/test/file{i}.py", "description": f"Test file {i} description" * 10}
        for i in range(1000)
    ]
...
def test_window_resize(result_ui, qtbot, helper):
    ...
    total_width = result_ui.result_table.columnWidth(
        0
...
def test_sort_functionality(result_ui, qtbot, helper):
    ...
    last_item = result_ui.result_table.item(
        result_ui.result_table.rowCount() - 1, 0
    ).text()
