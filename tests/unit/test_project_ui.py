...
def test_ui_components(project_ui):
    ...
    sections = [
        w
        for w in project_ui.findChildren(QFrame)
    ...
    assert {
        label.text() for label in labels if label.font().pointSize() == 24
    } == expected_titles
...
def test_error_handling(project_ui, qtbot, tmp_path, mocker):
    ...
    mocker.patch.object(Project, "__init__", side_effect=Exception("Test error"))
    ...
    mock_warning.assert_called_with(
        project_ui, "Error", "Failed to create project: Test error"
    )
...
def test_signal_connections(project_ui):
    ...
    assert (
        project_ui.start_dir_button.receivers(project_ui.start_dir_button.clicked) > 0
    )
    assert (
        project_ui.create_project_btn.receivers(project_ui.create_project_btn.clicked)
        > 0
    )
    assert (
        project_ui.load_project_btn.receivers(project_ui.load_project_btn.clicked) > 0
    )
...
def test_directory_access_error(project_ui, qtbot, tmp_path, mocker):
    ...
    mock_warning.assert_called_with(
        project_ui, "Invalid Directory", "Directory is not accessible"
    )
...
def test_file_operation_error(project_ui, qtbot, tmp_path, mocker):
    ...
    mock_warning.assert_called_with(
        project_ui, "Invalid Directory", "Invalid directory path: Access denied"
    )
...
def test_invalid_directory_path(project_ui, qtbot, mocker):
    ...
    mock_warning.assert_called_with(
        project_ui, "Invalid Directory", "Selected directory does not exist"
    )
