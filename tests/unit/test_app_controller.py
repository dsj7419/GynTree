...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QStatusBar, QWidget, QMessageBox
)
...
def test_result_ui(dashboard_ui, qtbot, helper):
    ...
    result = dashboard_ui.show_result(dashboard_ui.controller.project_context.directory_analyzer)
...
def test_exclusions_manager(dashboard_ui, qtbot, helper):
    ...
    result = dashboard_ui.manage_exclusions(dashboard_ui.controller.project_context.settings_manager)
...
def test_state_transitions(dashboard_ui, qtbot, helper):
    ...
    assert states[0]['buttons_enabled']['projects'], "Projects button should be enabled initially"
    assert not states[0]['buttons_enabled']['exclusions'], "Exclusions button should be disabled initially"
    ...
    assert states[1]['buttons_enabled']['exclusions'], "Exclusions button should be enabled after project creation"
    assert states[1]['window_title'] == f"GynTree - {project.name}", "Window title should reflect project name"
...
def test_error_recovery(dashboard_ui, qtbot, helper, mocker):
    ...
    def create_project_success(*args):
        ...
        dashboard_ui.controller.project_controller.project_context = MockProjectContext()
...
def test_ui_responsiveness(dashboard_ui, qtbot, helper):
    ...
    assert operation_time < 2.0, f"UI operations took too long: {operation_time:.2f} seconds"
...
def test_settings_persistence(dashboard_ui, qtbot, helper):
    ...
    assert new_dashboard.theme_toggle.isChecked() == (new_dashboard.theme_manager.get_current_theme() == 'dark')
