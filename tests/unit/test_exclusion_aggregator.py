...
def test_duplicate_handling(helper):
    ...
    assert len([item for sublist in aggregated['excluded_dirs'].values() 
                for item in sublist]) == 1
    assert len([item for sublist in aggregated['excluded_files'].values() 
                for item in sublist]) == 1
...
def test_path_normalization(helper):
    ...
    assert any(os.path.normpath('/path/to/dir') in items 
              for items in aggregated['excluded_dirs'].values())
    assert any(os.path.normpath('/path/to/file') in items 
...
def test_category_assignment(helper):
    ...
    assert 'other' in aggregated['excluded_dirs'], "Missing 'other' category in excluded_dirs"
    assert 'other' in aggregated['excluded_files'], "Missing 'other' category in excluded_files"
    ...
    assert any(os.path.normpath(p) == custom_folder_path 
              for p in aggregated['excluded_dirs']['other'])
    assert '.env' in aggregated['excluded_files']['config']
    assert any(os.path.normpath(p) == custom_txt_path 
...
def test_nested_paths(helper):
    ...
    exclusions = {
        ...
        'excluded_files': {
            '/path/to/root/file.txt',
            '/path/to/root/nested/file.txt'
        }
    }
    ...
    assert os.path.normpath(root_path) in {os.path.normpath(p) 
                                         for p in aggregated['root_exclusions']}
    assert 'other' in aggregated['excluded_dirs'], "Missing 'other' category"
    assert any(os.path.normpath(p) == os.path.normpath(nested_path) 
              for p in aggregated['excluded_dirs']['other'])
    assert any(os.path.normpath(p) == os.path.normpath(deeper_path) 
              for p in aggregated['excluded_dirs']['other'])
