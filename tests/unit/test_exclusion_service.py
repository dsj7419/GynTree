...
class TestExclusionServiceImpl(ExclusionService):
    ...
    def should_exclude(self, path: str) -> bool:
        ...
        # Handle mock tests
        if path in self._exclusion_cache and not hasattr(self.settings_manager, '_mock_return_value'):
...
def test_walk_directory_with_unicode_names(exclusion_service, tmpdir):
    ...
    paths = [
        os.path.join("🌟", "子目录", "подпапка"),
        os.path.join("τέστ", "測試", "테스트")
    ]
