...
class TestResourceManagement:
    def test_tree_exporter_temp_file_cleanup(self, tree_widget):
        ...
        temp_files = [tempfile.mktemp(suffix=".png"), tempfile.mktemp(suffix=".txt")]
    ...
    def test_directory_analyzer_memory_stability(self):
        ...
        with patch.object(
            analyzer.directory_structure_service,
    ...
    def test_project_context_resource_lifecycle(self, mock_project):
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "services.SettingsManager.SettingsManager.load_settings",
            return_value={"excluded_dirs": [], "root_exclusions": []},
        ):
            with patch.object(context, "initialize_directory_analyzer"), patch.object(
                context, "initialize_auto_exclude_manager"
    ...
    def test_tree_exporter_large_file_handling(self, tree_widget):
        ...
        try:
            with patch("PyQt5.QtGui.QPixmap.save", return_value=True), patch(
                "PyQt5.QtWidgets.QTreeWidget.render"
            ):  # Mock render to prevent GUI operations
                success = exporter._render_and_save_pixmap(
                    tree_widget, 800, 600, temp_path
                )
                assert success
        finally:
            os.unlink(temp_path)
