...
import time
import pytest
from PyQt5.QtCore import QThread, QThreadPool, QCoreApplication, QEvent, Qt
from PyQt5.QtTest import QSignalSpy
from unittest.mock import Mock, patch
from controllers.ThreadController import AutoExcludeWorkerRunnable, ThreadController, WorkerSignals
...
@pytest.mark.timeout(5)
def test_thread_cleanup_during_execution(thread_controller, mock_project_context, qtbot):
    for _ in range(3):
...
def test_thread_error_handling(thread_controller, mock_project_context, qtbot):
    mock_project_context.trigger_auto_exclude.side_effect = RuntimeError("Test runtime error")
...
def test_cleanup_during_error(thread_controller, mock_project_context, qtbot):
    mock_project_context.trigger_auto_exclude.side_effect = Exception("Cleanup test error")
    worker = thread_controller.start_auto_exclude_thread(mock_project_context)
    assert worker is not None
