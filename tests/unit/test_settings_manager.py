...
class SettingsTestHelper:
    ...
    def create_settings_file(self, project_name: str, settings: Optional[Dict[str, Any]] = None) -> Path:
...
def test_relative_path_handling(settings_manager, helper):
    ...
    relative_path = os.path.relpath(absolute_path, settings_manager.project.start_directory)
...
def test_wildcard_patterns(settings_manager, helper):
    ...
    # Prepare test paths
    test_file1 = os.path.join(settings_manager.project.start_directory, "logs", "deep", "test.log")
    test_file2 = os.path.join(settings_manager.project.start_directory, "temp", "folder1", "cache", "data.tmp")
