...
def mock_settings_manager(mocker):
    ...
    mock.get_root_exclusions = mocker.Mock(
        side_effect=lambda: test_data.get("root_exclusions")
    )
...
def mock_dialogs(mocker):
    ...
    mocker.patch.object(
        QFileDialog, "getExistingDirectory", return_value="/test/project/subfolder"
    )
    mocker.patch.object(
        QFileDialog, "getOpenFileName", return_value=("/test/project/test.txt", "")
    )
    return mocker
...
@pytest.fixture
def exclusions_ui(
    qtbot, mock_controller, mock_settings_manager, mock_theme_manager, mock_dialogs
):
    """Create the ExclusionsManagerUI instance with proper mocks"""
    ui = ExclusionsManagerUI(
        mock_controller, mock_theme_manager, mock_settings_manager
    )  # Use mock directly
...
def test_add_directory(exclusions_ui, qtbot):
    ...
    initial_count = len(
        exclusions_ui.settings_manager.get_all_exclusions()["excluded_dirs"]
    )

    add_dir_btn = next(
        btn
        for btn in exclusions_ui.findChildren(QPushButton)
...
def test_add_file(exclusions_ui, qtbot):
    """Test adding a file"""
    initial_count = len(
        exclusions_ui.settings_manager.get_all_exclusions()["excluded_files"]
    )
