...
class TestRunnerBase(abc.ABC):
    ...
    def _build_pytest_args(
        ...
        pytest_args.extend(
            [f"--timeout={timeout}", "--timeout-method=thread", "--timeout_func_only"]
        )
    ...
    def run(
        ...
        try:
            ...
            success = (
                ...
                and result_data.get("failed", 0) == 0
                and result_data.get("errors", 0) == 0
            )
...
class SingleTestRunner(TestRunnerBase):
    ...
    def run(
        ...
        try:
            ...
            return TestResult(
                success=success,
                message=f"Test {selected_tests[0]} completed successfully"
                if success
                else f"Test {selected_tests[0]} failed",
                duration=duration,
                failed_tests=result_data.get("failed_tests", []),
                warnings=result_data.get("warnings", []),
                error=result_data.get("error"),
